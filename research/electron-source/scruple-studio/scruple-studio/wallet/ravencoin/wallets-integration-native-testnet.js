...
// RVN wallet modules
const { WalletManager } = require('./wallet-testnet');
...
const { 
  ...
  getWalletBalance,
  ASSET_CREATION_COST 
} = require('./native-issuer-testnet');

// Arweave manager — testnet version (arlocal on Oracle VM)
const ArweaveManager = require('../arweave/arweave-index-testnet');
...
let _configManager = null;  // Retained for network get/set IPC handlers
...
function initializeWalletServices(walletDir, configDir, options = {}) {
  ...
  // Point the native issuer singleton at the correct network.
  // setNetwork() is async but we fire-and-forget here — the connection is
  // lazy (made on first actual use), so there is no race condition.
  nativeIssuer.setNetwork(network).catch(err =>
    console.error('[WALLETS] nativeIssuer.setNetwork error:', err.message)
  );
  ...
  if (!walletManager) {
    ...
    console.log('[WALLETS] WalletManager initialized (network:', network, ')');
  }
...
function setupWalletHandlers(ipcMain, configManager) {
  // Retain reference for network get/set handlers
  ...
  ipcMain.handle('arweave-import-key', async () => {
    try {
      ...
      if (result.success) {
        ...
        return { success: true, address: result.address, balance };
      }
      return result;
    } catch (error) {
      return { success: false, error: error.message };
  ...
  ipcMain.handle('arweave-get-status', async () => {
    console.log('[DEBUG] arweave-get-status handler called');
    
    const address = arweaveManager.getAddress();
    console.log('[DEBUG] address:', address);
    
    if (!address) {
      const result = { connected: false, error: 'No wallet' };
      console.log('[DEBUG] returning (no wallet):', JSON.stringify(result));
      return result;
    }
    
    const balanceResult = await arweaveManager.getBalanceWithStatus();
    console.log('[DEBUG] balanceResult:', JSON.stringify(balanceResult));
    
    if (!balanceResult.success) {
      const result = { connected: false, address, balance: 0, error: 'Network error' };
      console.log('[DEBUG] returning (network fail):', JSON.stringify(result));
      return result;
    }
    
    const result = { connected: true, address, balance: balanceResult.balance };
    console.log('[DEBUG] returning (success):', JSON.stringify(result));
    return result;
  });
  ...
  ipcMain.handle('arweave-mint-test-ar', async (event, address, winstonAmount) => {
    try {
      return await arweaveManager.mintTestAr(address, winstonAmount || 10000000000000);
    } catch (error) {
      return { success: false, error: error.message };
    }
  });
  ...
  // ===========================================================================
  // NETWORK SELECTION (Option A — config-driven, requires restart)
  // ===========================================================================

  ipcMain.handle('rvn-get-network', async () => {
    try {
      return { network: _configManager.getNetwork() };
    } catch (error) {
      return { error: error.message };
    }
  });

  ipcMain.handle('rvn-set-network', async (event, network) => {
    try {
      const saved = _configManager.setNetwork(network);
      return {
        success: saved,
        network,
        requiresRestart: true,
        message: `Network set to ${network === 'rvn-test' ? 'TESTNET' : 'Mainnet'}. Restart SCRUPLE Studio to apply.`
      };
    } catch (error) {
      return { success: false, error: error.message };
    }
