...
async function getConfig() {
  const response = await fetch(`${witnessUrl()}/api/stripe-config`, {
    method: 'GET',
...
async function createPaymentIntent(action, projectId, installationId) {
  const response = await fetch(`${witnessUrl()}/api/create-payment-intent`, {
    method: 'POST',
...
async function confirmAndExecute(paymentIntentId, action, projectId, installationId, options) {
  const response = await fetch(`${witnessUrl()}/api/confirm-and-execute`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      paymentIntentId,
