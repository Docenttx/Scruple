...
const { ElectrumXClient } = require('./electrumx-client-testnet');
const { 
  deriveMultipleKeys, 
  buildAndSignAssetTx,
  NETWORK_MAINNET,
...
class NativeAssetIssuer {
  /**
   * @param {'rvn'|'rvn-test'} network - Network to operate on (default: 'rvn' mainnet)
   */
  constructor(network = 'rvn') {
    this.network = network;
    this.networkParams = (network === 'rvn-test') ? NETWORK_TESTNET : NETWORK_MAINNET;
    this.electrumx = new ElectrumXClient(network);
    this.connected = false;
    
    if (network === 'rvn-test') {
      console.log('[NATIVE-ISSUER] Initialized for TESTNET');
    }
  }

  /**
   * Switch to a different network.
   * Disconnects existing ElectrumX connection and reconnects to new network.
   * 
  ...
  async setNetwork(network) {
    ...
    console.log('[NATIVE-ISSUER] Network switched to:', network);
  }
  ...
  async getBalance(mnemonic) {
    ...
    // Derive multiple addresses using correct network params
    const keys = deriveMultipleKeys(mnemonic, 20, this.networkParams);
...
// Network is set at startup via nativeIssuer.setNetwork() from wallets-integration-native.js.
const nativeIssuer = new NativeAssetIssuer('rvn');
