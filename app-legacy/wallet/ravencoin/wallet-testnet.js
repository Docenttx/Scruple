...
class WalletManager {
  constructor(walletDir, options = {}) {
    ...
    // Separate wallet files so mainnet and testnet wallets don't overwrite each other
    const walletFilename = (this.network === 'rvn-test') ? 'wallet-testnet.json' : 'wallet.json';
    this.walletPath = path.join(this.walletDir, walletFilename);
   ...
   */
  _loadDependencies() {
    // Load asset-encoder-testnet for correct testnet address derivation
    try {
      const assetEncoder = require('./asset-encoder-testnet');
      deriveKeyFromMnemonic = assetEncoder.deriveKeyFromMnemonic;
      this._networkMainnet = assetEncoder.NETWORK_MAINNET;
      this._networkTestnet = assetEncoder.NETWORK_TESTNET;
    ...
    // Load native-issuer-testnet for ElectrumX balance
    try {
      const nativeIssuer = require('./native-issuer-testnet');
  ...
  /**
   * Get the network params object for asset-encoder calls.
   * Falls back to inline defaults if asset-encoder wasn't fully loaded.
   */
  _getNetworkParams() {
    if (this.network === 'rvn-test') {
      return this._networkTestnet || { pubKeyHash: 0x6f, scriptHash: 0xc4, wif: 0xef,
        bip32: { public: 0x043587cf, private: 0x04358394 }, bech32: 'rc',
        messagePrefix: '\x16Raven Signed Message:\n' };
    }
    return this._networkMainnet || { pubKeyHash: 0x3c, scriptHash: 0x7a, wif: 0x80,
      bip32: { public: 0x0488b21e, private: 0x0488ade4 }, bech32: 'rc',
      messagePrefix: '\x16Raven Signed Message:\n' };
  }
  ...
  async createWallet(password) {
    ...
    // Derive address locally using correct network params
    const keyInfo = deriveKeyFromMnemonic(mnemonic, 0, this._getNetworkParams());
