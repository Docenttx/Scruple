...
async function executeLock(projectId, options) {
  ...
  let response;
  try { response = await fetch(witnessUrl() + '/api/fiat-chain-lock', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload), signal: AbortSignal.timeout(LOCK_TIMEOUT) }); }
