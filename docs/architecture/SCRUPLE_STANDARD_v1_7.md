...
### 9.2 EU-compliant watermarking (imperceptible mark, timestamp payload)

This modality is the peer of §9.1 for satisfying EU AI Act Article 50
Code of Practice Section 1 mandatory marking measures. It attaches an
imperceptible watermark to the output whose payload encodes a signing
timestamp, so downstream verifiers holding only the pixel or audio
data — with no metadata, no C2PA sidecar, no chain-lock lookup — can
still recover a Scruple-attributable marker.

The same embedder is applied per media type with the media-appropriate
...
**Scruple SCR_ID watermark.** When chain lock is selected on image,
video, or audio content, an accompanying watermark is embedded whose
payload structure is the same as §9.2 but whose body encodes the
Scruple ID (SCR_ID) plus an optional pinned-content hint rather than
a timestamp. Purpose: someone finding the file out in the wild — with
no metadata, no C2PA sidecar, no other verification path — can
extract the SCR_ID via the §9.2.5 detector, look it up in Scruple's
audit chain, and reach the chain-lock inscription that anchors its
provenance. The SCR_ID watermark is the in-band component of the
chain-lock modality; the ledger inscription itself is the
out-of-band component. Together they form a self-contained lookup
path independent of §9.1 and §9.2.

Chain lock is independent of §9.1 (C2PA) and independent of §9.2 (the
EU-compliant timestamp watermark). A customer may select chain lock
without either, and the resulting receipt still carries the full
underlying Scruple evidence-based provenance plus the SCR_ID
lookup path.
...
who selects (a) C2PA content credentials + (b) EU-compliant watermark
+ (c) chain lock receives an output that is: in-band signed as a C2PA
credential, watermarked with a timestamp payload, hash-anchored on the
public ledger, and additionally carries the Scruple SCR_ID watermark
attached automatically as part of the chain-lock modality — **four
independent verification paths** (C2PA manifest, timestamp watermark,
public-ledger inscription, SCR_ID lookup watermark), all inheriting
the same underlying attested-signer, baseline-bound witnessing.

A user selecting only (c) chain lock receives an output with Scruple's
underlying evidence-based provenance, the public-ledger anchor, and
the SCR_ID watermark — a self-contained lookup path independent of
C2PA and the timestamp watermark.

A user selecting only (a) C2PA and (b) EU-compliant watermark
receives the two Article 50 mandatory-measure paths without a
public-ledger anchor and without the SCR_ID lookup.
