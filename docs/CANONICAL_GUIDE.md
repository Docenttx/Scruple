...
- **What it signs:** every iteration with HMAC over the canonical record; every lock event with HMAC over `{project_id, action, merkle_root, witnessed_count, locked_at}`. Action is in the lock tuple so a checkpoint sig can't be replayed as a finalize sig.
