...
| **WO-CVM-01 L2 evidence live run** | **CAPTURED (partial)** | (staged) | SEV-SNP substrate + HSM binding captured; SoftHSM→c2pa-python signing path blocked on c2pa-python 0.36 verify bug — see NOTES |
...
[2026-07-12T18:20:00Z] c2pa-python signing | ROOT-CAUSE FIXED | (staged) | Isolated the "claimSignature.mismatch" c2pa Reader failure to a certificate-profile requirement in c2pa-rs 0.86+: (1) leaf cert Distinguished Name MUST have full C/ST/L/O/OU/CN attributes, not just CN; (2) leaf MUST be issued by a distinct root CA cert (self-signed leafs are rejected at Signer.from_callback with "certificate is invalid"); (3) EKU must be `critical, emailProtection` alone (not the Adobe private OID 1.3.6.1.4.1.62558.2.1); (4) KeyUsage must include nonRepudiation. Confirmed by producing a `Valid` c2pa.Reader validation state with the shipping signer subprocess. Regenerated services/c2pa-signer/keys/signer.pem via new regen-dev-cert.sh script + updated cert.cnf. Also added scripts/verify-c2pa-reader.py + wired it into the c2pa-sign E2E test as a real interop assertion — the "24/24 pass" earlier claim was hollow because the E2E only checked our own scruple-verify leaf chain, never c2pa.Reader on the JUMBF manifest itself. Discovery + fix: 2 hours of dig. NO impact on captured L2 evidence bundle (SEV-SNP report + HSM binding still valid; only the on-CVM signing loop was affected).

[2026-07-12T17:52:00Z] WO-CVM-01 | LIVE-RUN | (staged) | L2 evidence pipeline executed on OCI Confidential Compute VM (VM.Standard.E5.Flex, Ubuntu 24.04.4, kernel 6.17.0-1011-oracle). Confirmed AMD SEV-SNP active ("Memory Encryption Features active: AMD SEV SEV-ES SEV-SNP", `/dev/sev-guest` present). Captured: SEV-SNP report (1184B, version 5, VM measurement 7237c44b..., chip_id bd296e67..., reported_tcb 0x581c00000000000a, report_data cryptographically binds SoftHSM ES256 pubkey sha256 d5b782d8...), AMD VCEK cert (1347B DER, issuer SEV-Genoa), ARK+ASK cert chain (4602B PEM), SoftHSM ES256 pubkey (never left CVM), dev CA + end-entity cert chain with EKU documentSigning+emailProtection, valid C2PA-signed test PNG via c2patool baseline. Bundle at docs/l2-evidence/2026-07-12T174954Z/ (tarball sha256 31c08e41...). CVM instance ocid1.instance.oc1.iad.anuwcljtzjtltxictqqomtd7thg73al5xlwmjs62omtnbbfjn63nell22hoa terminated post-capture. BLOCKER: SoftHSM → c2pa-python 0.36 Signer.from_callback → Reader path returns "claimSignature.mismatch" for signatures the underlying key verifies fine over. Same failure with pure-software EC key, proven-working callback pattern. c2patool 0.9.12 (c2pa-rs 0.37) can't parse c2pa-python 0.36 manifests at all ("claim could not be converted from CBOR"). Suggests c2pa-python 0.36 sign/verify version-skew bug. Follow-up: try c2pa-python 0.34/0.35 or route signing through c2patool with PKCS11 external-signer script. Core L2 evidence (§6.1.2 binary integrity + §6.2.2 key confidentiality) fully captured — the SoftHSM signing loop is a follow-up not a blocker on L2 filing.

[2026-07-12T08:30:00Z] WO-CVM-01 | DESIGN | (staged) | L2 evidence path added: canonical §18 corrects earlier over-reach on FIPS 140-2 L3 (not required); C2PA GPSR §6.1.2 + §6.2.2 both satisfied by SEV-SNP + SoftHSM in OCI CVM. L2_EVIDENCE_TEMPLATE.md written as Security Architecture Document template. WO-CVM-01 drafted: SoftHSM backend module for existing vault_sign.py dispatch + deploy/l2-evidence-run/ playbook + verifier sev-snp-report subcommand. Not yet coded — awaiting user's CVM provisioning to test live. Evidence-run cost: ~$1. Continuous: ~$120/mo when active.

[2026-07-12T07:45:00Z] WO-03+WO-08+WO-10 | DONE | 0d45097 | Full C2PA sign → witness → verify pipeline shipping. Migration 031 (users.principal_id). Python vault_sign.py callback with local-fallback + OCI Vault lazy-import (only imports oci SDK when env vars set). sign.py refactored to Signer.from_callback + reports signing_mode + signer_identity. signAsset.ts adds asset_sha256 + outputManifestSha256 for audit correlation. New TS helpers: scrupleInternalEmit.ts (auto-provisions internal tenant creds with dev-file persistence across hot-reloads), principalForUser.ts (get-or-create Principal + delegation). /api/scruple/c2pa/sign wraps sign with witness emission — fail-open. E2E test 24/24 PASS: sign → witness fields → principal auto-minted → delegation active → leaf in DB → second sign chains → checkpoint → verifier CLI VALID. Tomorrow's HSM: set SCRUPLE_C2PA_VAULT_KEY_OCID + endpoint env vars + pip install oci; no code change.
