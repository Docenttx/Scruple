...
For the Project 181 LoRA the sidecar is 16,498 bytes. Sidecar SHA-256 is `0027d50242c8439028ad3e1833882e7603745f8738a59525abf9ba66fb7bd2dd`. Manifest first 16 bytes are `000040726a756d620000001e6a756d64`, which is the JUMBF `jumb ` + `jumd ` header sequence — a decomposer can confirm the container shape without a C2PA library.
...
**Step 1 — Hash the model.** Compute `sha256sum stay-puft-cyberpunk-lora-r4.safetensors` and confirm the result equals `31e35c80fc4829d14f90153f4c74cd59c90b779f6afe05a74cd6120b893f7e5b`. This is the value stored in the sidecar's `c2pa.hash.data.hash` CBOR field.
