...
Scruple satisfies the multi-layered requirement of Section 1 with the following three sub-measures on AI-generated outputs produced through our infrastructure:
...
- **Sub-measure 1.1.2 — Imperceptible watermark (raster image outputs).** At publication time, every raster image output published through Scruple Studio carries a 128-bit imperceptible watermark embedded via classical DCT spread-spectrum in the mid-frequency coefficients of the Y-channel, protected by Reed-Solomon ECC. The watermark encodes scheme version, tier, and either a timestamp (self-contained tiers) or a public-ledger Ravencoin asset identifier (chain-lock tiers). The clean master is preserved unmodified; the watermark is applied to a derivative alongside for public distribution. The reference implementation is open at `services/watermark/`. Video + audio watermarking follows in Phase 2 per the roadmap in §6.

- **Sub-measure 1.1.3 — Fingerprinting / logging.** Every output additionally emits a per-iteration cryptographic leaf into the Scruple witness chain. Leaves are Merkle-chained and the root is anchored to a public ledger on a fixed cadence. This is a fingerprint+logging method in the Recital 133 sense that persists even if the C2PA manifest is stripped downstream — a documented C2PA v2.0 failure mode that the third layer directly addresses.
...
- Q3 2026: expose the Sub-measure 1.1.1 signing surface to downstream vendors via the same Witness API path our integrations use; publish `witness-integration.md` as normative for third parties — done in draft form.
- Q3 2026: ship Sub-measure 1.1.2 (imperceptible watermark) for raster image outputs of Scruple Studio at publication time — done (see §3 above; reference implementation open-source at `services/watermark/`).
- Q4 2026: extend Sub-measure 1.1.2 coverage to video (per-frame with GOP-level payload rotation) and audio (frequency-domain spread-spectrum); evaluate SynthID drop-in for base-model-side integration where SynthID model-family availability aligns with the base models we deploy.
