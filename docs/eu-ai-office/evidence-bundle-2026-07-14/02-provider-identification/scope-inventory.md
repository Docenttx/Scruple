...
|---|---|---|---|---|---|---|---|
| 1 | Stable Diffusion 1.5 (Runway / Stability AI upstream) | Scruple Studio Web (`scruple.ai`) | image | 1.1.1 C2PA v2.x + 1.1.3 Merkle-anchored witness leaf | C2PA v2.x; ISO/IEC 21617-1:2025 compatible manifest engine | `scruple-verify` (npm, open source); planned public verify URL per §6 of cover letter | 1.1.1 shipped; 1.1.3 shipped; 1.1.2 in Q4 2026 SynthID eval |
...
| 7 | Third-party AI systems using Scruple Witness API | `witness.scruple.ai` public API | image, video, audio, text-adjacent artefacts (as reported by caller) | 1.1.1 signature + 1.1.3 witness leaf, both applied by Scruple to hashes supplied by caller | C2PA v2.x; Scruple Standard v1.2 | Same public verify URL when it publishes | 1.1.1 shipped; caller assertion; 1.1.3 shipped |
...
- **Robust and reliable as far as technically feasible** — the 1.1.3 witness Merkle chain addresses C2PA's documented manifest-stripping failure mode.
