# Runbook — Provision a Production SEV-SNP CVM for the Scruple C2PA Signer

**Cited from:** `01-GPSA.md` §C.2.2, §C.2.3, §C.2.5, §C.2.6.
**Audience:** Scruple operator with tenancy-admin OCI credentials and access to the DigiCert / SSL.com Trust List CA account.
**Goal:** bring a fresh SEV-SNP-attested CVM online, generate an ES256 signing key inside SoftHSM, cryptographically bind that key to the CVM's SEV-SNP attestation, and land the `scruple-c2pa-signer.service` daemon ready to accept mTLS sign requests from Backend-Web.
**Wall time:** ~30 minutes hands-on, ~60 minutes end-to-end including CVM boot and health-check settle.
**Companion runbook:** `cert-enrollment.md` runs immediately after this one and produces the production Trust-List end-entity cert.
...
## 12. Rotation — annual or on suspected compromise

SEV-SNP-sealed SoftHSM keys are non-recoverable by design. Rotation is a full re-provision:

1. Provision a new CVM per §1–§7 with display name `scruple-c2pa-signer-prod-0N` where `N = previous + 1`.
2. Run `cert-enrollment.md` end-to-end on the new CVM. Publish the new cert under a new trust-manifest topology entry (§10), leaving the old topology entry in place with `deprecated_at` set to `activated_at + 30d` per §4.3 of `CANONICAL_SCRUPLE_WITNESSING_L2.md`.
3. Cut Backend-Web's DNS for `sign.scruple.internal` over to the new CVM's private IP.
4. Watch a checkpoint interval for the C2PA sign leaves to land under the new signer identity; confirm `scruple-verify c2pa` still validates historical signs under the OLD trust-manifest entry.
5. After the 30-day overlap window, terminate the old CVM (`oci compute instance terminate --instance-id ...`). The SoftHSM key is destroyed with the CVM — no key-material teardown ceremony needed.
6. Update `docs/l2-evidence/` with the new evidence bundle produced during the new CVM's ceremony.

**On suspected compromise:** collapse the overlap to zero — set the old topology's `deprecated_at` to `now`, force-invalidate any in-flight signs to Backend-Web, and re-issue the trust manifest with only the new entry active. Notify affected Principals per the Rider §8 remediation flow.
