...
export default function CanvasBridge({
  ...
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      ...
      // Canvas v2 (WO-4) note: the `scruple:canvas-witness-{start,complete}`
      // postMessage protocol from the on-host canvas-witness.js intercept
      // is RETIRED. Canvas v2 routes all canvas traffic through the
      // scruple-web HTTP+WS proxy at /canvas-proxy/<sessionId>/, which
      // captures workflow JSON + output bytes server-side. The browser
      // no longer participates in provenance capture.
    }
